...
*Note: Specifying preemption behavior of tasks through `production` flag is deprecated in favor of
  ...
  ```environment``` | String | Job environment, default ```devel```. By default must be one of ```prod```, ```preprod```, ```devel```, ```test``` or ```staging<number>``` but it can be changed by the Cluster operator using the scheduler option `allowed_job_environments`.
